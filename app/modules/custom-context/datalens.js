        ...
        authorization: function (data, callback) {
            ...
            accessFilter.filter(session.user.isEmbed ? 'DL.PREVIEW' : 'DL', data, session.user.id, schemas, function (err, rows) {
                if (rows) {
                    ...
                    callback(result_layout.ok([{ 'statusCode': 200, 'token': session.request.headers['rpc-authorization'], user_id: session.user.id, claims: session.user.c_claims, projectId: session.user.c_project_name || args.application_name, login: session.user.c_login, root: session.user.c_claims.indexOf(args.primary_role) >= 0 }]));
                } else {
