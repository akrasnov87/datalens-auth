...
if [ -n "$NODE_VPATH" ]; then
    ...
    sed -i 's+virtual_dir_path="/\w*/"+virtual_dir_path="/'"$CONF"'/"+g' $CONFIG_PATH
fi

sed -i 's+application_name="datalens-auth-\w*-node"+application_name="datalens-auth-'"$CONF"'-node"+g' $CONFIG_PATH
